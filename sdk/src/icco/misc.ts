...
export async function getAcceptedTokenDecimalsOnConductor(
  ...
  if (contributorChain !== conductorChain) {
    ...
    if (originalToken.chainId === conductorChain) {
      ...
      const nativeConductorAddress = tryUint8ArrayToNative(originalToken.assetAddress, originalToken.chainId);

      if (nativeConductorAddress !== undefined) {
        // fetch the token decimals on the conductor chain
        tokenDecimalsOnConductor = await getErc20Decimals(conductorProvider, nativeConductorAddress);
      } else {
      ...
      if (conductorWrappedToken !== null) {
        // fetch the token decimals on the conductor chain
        tokenDecimalsOnConductor = await getErc20Decimals(conductorProvider, conductorWrappedToken);
      } else {
...
export async function normalizeConversionRate(
  ...
  if (acceptedTokenDecimals === conductorDecimals) {
    ...
    return normalizedConversionRate.div(parseUnits("1", acceptedTokenDecimals - conductorDecimals));
  } else {
    return normalizedConversionRate.mul(parseUnits("1", conductorDecimals - acceptedTokenDecimals));
  }
}
